...
class MultipleOutputLoss2_ME(nn.Module):
    def __init__(self, loss, weight_factors=None):
        ...
        """
        super(MultipleOutputLoss2_ME, self).__init__()
    ...
    def forward(self, x, y, class_for_batch=torch.arange(10)):#class index接入口
        ...
        for batch in range(x[0].shape[0]):#x[0]:[BCDHW]ori_res
            if len(class_for_batch[batch]) > 9:#全标签
                ...
                l = self.loss(x[0][batch].unsqueeze(dim=0), y[0][batch].unsqueeze(dim=0), class_for_batch[batch])
            L += l
