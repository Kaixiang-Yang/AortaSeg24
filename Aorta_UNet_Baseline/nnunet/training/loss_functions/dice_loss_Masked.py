...
class Masked_SoftDiceLoss(nn.Module):
    ...
    def forward(self, x, y, loss_mask=None, class_for_batch=None):
        shp_x = x.shape
        ...
        # if not self.do_bg or self.do_bg:#保险起见，无论是否do_bg，Masked Dice Loss都不计算背景
        #     if self.batch_dice:
        #         dc = dc[1:]
        #     else:
        #         dc = dc[:, 1:]
        #因为基本不计算背景，所以此处未考虑错误的背景
...
class Masked_CrossEntropy(nn.Module):
    ...
    def forward(self, logit, target, class_for_batch):#体素水平下的计算
        # 1.softmax
        # 2.log
        # 3.gt赋权重
...
class DC_and_CE_loss_Masked(nn.Module):
    ...
    def forward(self, net_output, target, class_for_batch):
        ...
        else:#考虑到若是排除所有背景，计算ce_loss时分母小，因此loss大，难以训练和难收敛，因此missing时也使用正常的CEloss，这也反映了MEloss的好处
            dc_loss = self.Masked_dc(net_output, target, loss_mask=mask, class_for_batch=class_for_batch) if self.weight_dice != 0 else 0
            ce_loss = self.normal_ce(net_output, target[:, 0].long()) if self.weight_ce != 0 else 0
            # ce_loss = self.Masked_ce(net_output, target, class_for_batch=class_for_batch) if self.weight_ce != 0 else 0
