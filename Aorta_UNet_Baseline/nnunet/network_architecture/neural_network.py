...
class SegmentationNetwork(NeuralNetwork):
    ...
    def _compute_steps_for_sliding_window(patch_size: Tuple[int, ...], image_size: Tuple[int, ...], step_size: float) -> List[List[int]]:
        ...
        steps = []
        for dim in range(len(patch_size))[0:1]:
        ...
        # for y, x, we consider [img//2-patch,img//2-patch*stepsize,img//2]
        if image_size[1] > 2.0 * patch_size[1]:
            ...
            steps_here = sorted(list(set(steps_here)))
        else:
            steps_here = [0]
        steps.append(steps_here)
    ...
    def _internal_predict_3D_3Dconv_tiled(self, x: np.ndarray, step_size: float, do_mirroring: bool, mirror_axes: tuple,
        ...
        """
        For AortaSeg 2024, we only infer sliding window on GPU, 
        i.e., data[None, :, lb_x:ub_x, lb_y:ub_y, lb_z:ub_z] not whole data
        """
        ...
        if all_in_gpu:
            ...
            if use_gaussian and num_tiles > 1:
                ...
                add_for_nb_of_preds = torch.ones(patch_size, device="cpu")
        ...
        for x in steps[0]:
            ...
            for y in steps[1]:
                ...
                for z in steps[2]:
                    ...
                    if all_in_gpu:
                        predicted_patch = predicted_patch.half().cpu().numpy()
                    else:
